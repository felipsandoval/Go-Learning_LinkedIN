...
// To support custom types we need to use a ~ char. Underlying types.
func sumCustom[T ~int | ~float64](a, b T) T {
	return a + b
}
...
func main() {
	...
	one := specialInt(1)
	two := specialInt(2)
	fmt.Println(sumCustom[specialInt](one, two))

}

// list is a singly-linked list that holds values of any type
// This has value for ANY types.
